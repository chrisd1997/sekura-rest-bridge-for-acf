	...
	class Sekura_ACF_API {
		...
		public function get_fields( $request ) {
			...
			if ( $this->get_id( $request ) ) {
				...
				if ( $this->get_field_objects( $this->id ) ) {
					if ( $field ) {
						$this->filter_show_in_rest( $data, $field, $this->field_objects );
					} else if ( is_array( $data['acf'] ) ) {
						foreach ( array_keys( $data['acf'] ) as $key ) {
							$this->filter_show_in_rest( $data['acf'], $key, $this->field_objects );
						}
					}
				}
			} else {
		...
		public function edit_in_rest( $field ) {
			if ( isset( $field['edit_in_rest'] ) && ! $field['edit_in_rest'] ) {
				return false;
			}
			return true;
		}
